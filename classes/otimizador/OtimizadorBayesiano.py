...
class OtimizadorBayesiano:
    ...
    def otimizar_offline(self, modelo_classe, espaco_parametros, X, Y):
        """
        Otimiza um modelo offline usando busca bayesiana
        """
        # Se o modelo for uma classe, crie uma instância
        if isinstance(modelo_classe, type):
            modelo = modelo_classe()
        else:
            modelo = modelo_classe

        # Importante: BayesSearchCV espera um estimator scikit-learn,
        # não nosso wrapper. Precisamos acessar o estimator interno.
        if hasattr(modelo, 'modelo'):
            estimator = modelo.modelo
        else:
            estimator = modelo  # Caso não seja nosso wrapper

        otimizador = BayesSearchCV(
            estimator=estimator,  # Use o estimator scikit-learn interno
            search_spaces=espaco_parametros,
            n_iter=self.n_iter,
            cv=self.cv,
        ...
        # Ajustar o otimizador
        otimizador.fit(X, Y)

        # Construir modelo otimizado
        if hasattr(modelo, 'modelo'):
            # Se for nosso wrapper, atualizar o modelo interno
            modelo.modelo = otimizador.best_estimator_
        else:
            # Se não for wrapper, retornar diretamente
            modelo = otimizador.best_estimator_

        return modelo, otimizador.best_params_
